...
The current release was evaluated using eleven artifacts:
\path{ontology.ttl}, \path{artifacts/cost_tuples.csv},
\path{artifacts/incident_tuples.csv}, \path{artifacts/cq_queries.sparql},
\path{artifacts/objective_weights.csv}, \path{artifacts/constraints.shacl.ttl},
\path{artifacts/cq_results.csv}, \path{artifacts/voi_priorities.csv},
\path{artifacts/objective_comparisons.csv},
\path{artifacts/shacl_results.csv}, and
...
horizon, bearing mode, decision-maker, evidence grade, data origin, source key,
source locator). Distribution is: 59 E1, 39 E2, and 22 E3 rows; 59 Measured, 39
...
confidence, provenance, attribution-evidence, linkage-mechanism, and
counterfactual-effect fields plus explicit residual-risk bearers
(8 E2/Inferred and 4 E3/Synthetic).

\subsection{Competency-Question Coverage Results}
All eight CQs are executable in the current release through generated artifacts.
CQ3 is now objective-function-driven with explicit baseline families, CQ4
includes attribution/linkage/counterfactual subfields, CQ7 adds
entailment-style consistency checks, and CQ8 enforces explicit opportunity-cost
foregone-alternative descriptors.
This means the burden-allocation instrument framing is no longer only
conceptual: it is executable at artifact level.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{0.18\columnwidth}p{0.10\columnwidth}p{0.60\columnwidth}}
    ...
    CQ2 (transfer) & Pass & 120/120 tuples include bearing mode; transfer-target coverage 54/54. \\
    CQ3 (comparison) & Pass & 2/2 objectives include explicit weights + baseline and produce ranked deltas. \\
    CQ4 (attribution) & Pass & 12/12 incident tuples include loss/confidence/provenance + attribution evidence + linkage mechanism + counterfactual effect + residual-risk bearer. \\
    ...
    CQ7 (entailment) & Pass & 0 internalized contradictions; 0 transfer-target contradictions; 120 inferred mechanism-bearer pairs. \\
    CQ8 (opportunity) & Pass & 20/20 OpportunityCost tuples include explicit alternative-use + foregone resource, benefit, and design constraint. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table*}[t]
  ...
  \begin{tabular}{p{0.07\textwidth}p{0.44\textwidth}p{0.41\textwidth}}
    ...
    RQ4 & Cross-family comparison is feasible when mechanisms are scored under explicit objective weights and a declared baseline. & CQ3 pass for both objective profiles with ranked family deltas in \texttt{artifacts/objective\_comparisons.csv}. \\
    \bottomrule
  \end{tabular}
\end{table*}

\paragraph{Decision-delta and reactive coverage.}
...
More broadly, it shows that mechanisms with similar local technical profiles can
still behave as different burden-allocation instruments.

\paragraph{Sensitivity results (CQ5).}
...
\paragraph{Consistency and opportunity results (CQ7--CQ8).}
SHACL-equivalent consistency checks report zero contradictions for internalized
bearer alignment and transfer-target distinctness; detailed rule outcomes are in
\texttt{artifacts/shacl\_results.csv}. Opportunity-cost explicitness passes with
20/20 rows carrying explicit alternative-use identifiers plus foregone
resource, foregone benefit, and design-constraint fields.

\paragraph{Calibration of claims.}
...
effect sizes. The current evidence supports the framing claim at method level
(queryability, decision-delta potential, and allocation visibility), while
leaving benchmark-grade effect estimation to future corpus expansion.

\paragraph{Reproducibility outputs.}
Running \texttt{make repro} regenerates CQ/VOI/objective/SHACL/sensitivity
artifacts and writes \texttt{artifacts/repro\_report.txt} with tuple counts,
distribution checks, and CQ status summaries used in this section.
