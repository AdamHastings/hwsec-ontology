...
provides consistency and inference checks.
...
externalized, and to whom. CQ3 asks how mechanisms differ in burden
distribution and opportunity cost for a fixed security objective. CQ4 asks
which observed incident losses are plausibly linked to missing or weakly
adopted controls, with confidence annotation. CQ5 asks which uncertain
assumptions most affect comparative conclusions. CQ6 asks which missing
measurements would most reduce decision uncertainty.
...
(internalized/transferred/externalized). CQ3 maps to shared
threat/requirement context with comparable burden vectors. CQ4 maps to
incident-event entities linked to control absence/weakness, observed losses,
and attribution confidence. CQ5 maps to uncertainty annotations and repeatable
perturbation analysis over E2/E3 inputs. CQ6 maps to executable
value-of-information (VOI) ranking over family-cost cells using uncertainty,
transfer, and incident-link signals.
...
from the decision-maker. A property chain (\textit{incursCost} followed by
\textit{borneBy}) implies a mechanism-level \textit{hasCostBearer} relation.
...
paradigm claim specifies rejection conditions).
...
through CQ6 in the companion artifact package and an artifact-generation script
...
\texttt{artifacts/voi\_priorities.csv}, and
